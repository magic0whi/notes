...
\dd x=\lim_{\Delta x\to0}\Delta x.
\end{gather*}
\begin{proof}[$\dd x=\lim_{\Delta x\to0}\Delta x$]
  Substitute $f(x)=x$ into both equations gets
  \begin{align*}
                                  \dd x&=(x+\dd x)-x\\
    \equiv\lim_{\Delta x\to0}\Delta x&=\lim_{\Delta x\to0}(x+\Delta x)-x\\
                \therefore\mathrm{d}x&=\lim_{\Delta x\to0}\Delta x.
  \end{align*}
\end{proof}
\subsection{Properties of Leibniz Notation}
\begin{itemize}
\item Units: If $P$ has units of pressure, and $t$ has units of \textit{time}, then $\frac{\dd P}{\dd t}\) has units of pressure per time.
