...
\begin{proof}
    ...
    \]
    Thus we can replace \(\Delta x\) with \(\mathrm{d}x\) from the $\Delta f(x)$ and remove the limit symbol, then naturally $\Delta f(x)$ becomes $\mathrm{d}f(x)$, Perfect!\par
...
\section{Inverse Function}\todo{Progess is here}
\begin{enumerate}
