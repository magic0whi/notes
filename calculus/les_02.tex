...
\begin{proof}
  To proof the product rule, we can write $f(x+\dd x)=\mathrm{d}f(x)+f(x)\), the reasons why this equation is important are that if we differentiate the \(h(x)=f(x)g(x)\):
    \begin{align*}
