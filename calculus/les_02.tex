...
\todo{Add geometric proof}% https://openlearninglibrary.mit.edu/courses/course-v1:MITx+18.01.1x+2T2019/courseware/unit2/diff_2-sequential/?child=first
\begin{proof}
  ...
  \end{align*}
  The $\dd f\dd g/\dd x\) can be transformed into $\dd f/\dd x\cdot\dd g/\dd x\cdot\dd x$ which would be 0 because the extra $\dd x$ at the end. Finally
...
\todo{Add a proof without using the chain rule}% https://openlearninglibrary.mit.edu/courses/course-v1:MITx+18.01.1x+2T2019/courseware/unit2/diff_3-sequential/?child=first
...
\todo{Add another way to proof, tip: proof $e^{\lim_{n\to\infty}(1+\frac{1}{n})^n}=e^1$ (with limit to the form $\frac{\ln(1+\dd x)-\ln1)}{\dd x}$}
...
\todo{Add example to differentiate $x^x$}
